...
module.exports = [
  ...
  },
  {
    ...
    'script': function(boombot, text, uname, uid, private) {
      if (boombot.config.queue) {
        if (boombot.queue)
          boombot.respond(uid, 'Queue is on. Song limit is:'+boombot.queueLength+'. addme to join. q- to leave. q to see the current wait. /plays for dj played counts', private)
        else
