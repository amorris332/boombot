...
/**///  }
  ...
  if (text.match(/skip/i)) {
    boombot.respond(uid, 'Yeah, this sucked anyway.', private)
    boombot.bot.skip()
  }
  ...
  if (text.match(/dance/i)) {
    boombot.bot.bop()
    boombot.respond(uid, 'Ugh.', private)
  }
  // set the bots avatar
  ...
  if (text.match(/vampire up/i)) {
    try {
      ...
      boombot.respond(uid, 'Still a better love story than twilight.', private)
    } catch (err) {
      boombot.respond(uid, 'I do not have that form master.', private)
    }
  }
  ...
  if (text.match(/addsong/i)) {
    boombot.bot.roomInfo(true, function(data2) {
      try {
        ...
        boombot.respond(uid, 'Added '+newSongName+' to the crates.', private)
      } catch (err) {
        errMsg(err)
      }
    })
